...
public class ResolutionMethod {
     ...
     * Метод принимает КНФ, если термы не приведены в эту форму,то метод будет работать некорректно
     * @param CNF
     * @return
     */
    static public Pair<Boolean, Set<Term>> solve(List<Term> CNF) {
        Set<Term> update = new HashSet<>();
        Set<Term> current = new HashSet<>();
        ...
        } while(!update.isEmpty()); // цикл выполняется до тех пор, пока не перестанет пополнятся множество
        Pair<Boolean, Set<Term>> res = new Pair();
        res.first = isFoundZero;
        res.second = current;
        return res;
    }
     ...
     * @return
     */
    static public Term generateResolvent(Term a, Term b) {
        // Получаем списки простых высказываний
        List<Term> aSimple = a.getSimple();
        List<Term> bSimple = b.getSimple();
        ...
        for (Term as : aSimple) {
            for (Term bs : bSimple) {
                if ((as.getName().equals(bs.getName())) && (as.isNeg() != bs.isNeg())) {
                    // Если мы нашли одинаковые простые высказыванияс, но отлиные по статусу отрицания
...
//                    break;
        ...
        if (isCreate) {
            ...
            if (terms.isEmpty()) {
                // Если множество просых высказываний путсо, то мы получили нулевой дизъюнкт
     ...
     * @param terms
     */
    public static void simplify(Set<Term> terms) {
        ...
        for (Term f : terms) {
            for (Term s : terms) {
                ...
                if(f.getName().equals(s.getName()) && f.isNeg() != s.isNeg()) {
                    remove.add(f);
                    remove.add(s);
                }
            }
